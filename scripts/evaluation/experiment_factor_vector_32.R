...
mod <- load_model_tf("keras_tuner/best_models/experiment_factor/vector_32")
test_set <- read_csv("data/experiment_factor/vector/32/test.csv")

test_set$pred <- mod$predict(test_set[,1:32]) %>% as.numeric()
...
lineup_dat <- map(vi_lineup, function(lineup) {
  this_dat <- lineup$data %>%
    filter(null == FALSE)
  residual_to_vector(this_dat$.resid, this_dat$.fitted, 32)
}) %>%
  reduce(rbind) %>%
  `rownames<-`(NULL) %>%
  as.data.frame() %>%
...
lineup_dat$pred <- mod$predict(lineup_dat[,1:32]) %>% as.numeric()
